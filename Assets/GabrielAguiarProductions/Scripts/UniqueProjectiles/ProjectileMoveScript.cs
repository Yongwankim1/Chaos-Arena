...
public class ProjectileMoveScript : MonoBehaviour {
    ...
    [Tooltip("Delay in seconds before projectile starts moving (allows muzzle effect to play)")]
    public float buildUpTime = 0;
    public GameObject muzzlePrefab;
    public Vector3 offsetHit = Vector3.zero;
    ...
    void Start () 
    {
        ...
        // Store original speed and temporarily set it to 0
        originalSpeed = speed;
        speed = 0;
        ...
        // Hide the projectile and schedule it to become active after delay
        if (buildUpTime > 0)
        {
            gameObject.SetActive(false);
            Invoke("ActivateProjectile", buildUpTime);
        }
        else
        {
    ...
    void FixedUpdate () 
    {
        // Only move if build-up delay has finished
        if (canMove)
        {
            if (target != null)
        ...
        if (!bounce)
        {
            if (co.gameObject.tag != "Bullet" && !collided)
            {
                ...
                if (hitPrefab != null)
                {
                    ...
                    if (alignWithNormals)
                        hitVFX = Instantiate(hitPrefab, pos + offsetHit, rot);
                    else
                        hitVFX = Instantiate(hitPrefab, pos + offsetHit, Quaternion.identity);
